...
/// Represents a set of possible moves and has the ability to select
/// one of them by different means.
/// The representation allows for one possible move per column.
/// So a maximum of 7 moves can be stored in one MoveSet!
pub struct MoveSetIterator {
    moveset: u64,
    current_column: u8,
}

impl Iterator for MoveSetIterator {
    type Item = u64;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let selection = self.move_at_column(self.current_column);
            if selection > 0 {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        loop {
            ...
            // Iterate up to the nth found move in the moveset
            if selection > 0{
                if n == 0 {
                    return Some(selection);
                }
...
impl MoveSetIterator {
    /// Takes all the possible moves encoded in the given u64
    /// and constructs a MoveSetIterator from them
    pub fn from_moves(moves: u64) -> MoveSetIterator {
        MoveSetIterator {
            moveset: moves,
            current_column: 0,
        }
    ...
    pub fn move_at_column(&self, row: u8) -> u64 {
        let mask = BitBoard::FIRST_COLUMN << (row * 8);
        let selection = self.moveset & mask;
        selection
    }
}
...
impl BitBoard {
    ...
    pub fn play_field(&self, field: u64) -> BitBoard {
        debug_assert_eq!(field.count_ones(), 1);
