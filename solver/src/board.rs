...
impl BitBoard {
    ...
    pub fn is_move_valid(&self, column: u8) -> bool {
        ...
        BitBoard::move_in_col(all_valid, column as u64) > 0
    }

    pub fn play_column(&self, column: u8, player: FieldType) -> Option<BitBoard> {
        let all_valid = self.all_possible_moves();
        let mov = BitBoard::move_in_col(all_valid, column as u64);
...
impl BitBoard {
    ...
    #[inline]
    pub fn move_in_col(possible_moves: u64, column: u64) -> u64 {
        let column_mask = BitBoard::FIRST_COLUMN << (8 * column);
        column_mask & possible_moves
    }

    /// Returns the y position of the stone on the board when played in the given column.
    /// Returns None if the move is not possible
    pub fn stone_position(&self, column: u64) -> Option<usize> {
        assert!(column < BOARD_WIDTH as u64, "The given column has to be inside of the board to be checked for validity.");
        //todo: test this brainfuck code
        let moves = self.all_possible_moves();
        let in_row = Self::move_in_col(moves, column);
        if in_row == 0 {
            return None;
        }
        for i in 0..BOARD_HEIGHT {
            if (in_row >> (8 * (column))) >> i == 1 {
                return Some(i);
            }
        }
        None
    ...
    /// Returns the row index of the specific move, given a specific column.
    /// The move is given in the typical bit pattern
    pub fn stone_row(bit_move: u64, column: u64) -> Option<usize> {
        assert!(column < BOARD_WIDTH as u64, "The given column has to be inside of the board to be checked for validity.");

        let in_row = Self::move_in_col(bit_move, column);
        if in_row == 0 {
            return None;
        }
        for i in 0..BOARD_HEIGHT {
    ...
    /// Converts the move given in bitboard representation into real board coordinates
    pub fn move_to_coords(bit_move: u64) -> Option<(u32, u32)> {
        for col in 0..BOARD_WIDTH {
            let row = Self::stone_row(bit_move, col as u64);
            if let Some(row) = row {
                return Some((col as u32, row as u32));
            }
        }
        None
    }
